...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'Joerg Zdarsky';
    var email     = grunt.option('email')  || 'joerg.zdarsky@gmx.de';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new syncthing name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/syncthing.png')) {
            fs.renameSync(__dirname + '/admin/syncthing.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/syncthing.html')) {
            fs.renameSync(__dirname + '/widgets/syncthing.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/syncthing/js/syncthing.js')) {
            fs.renameSync(__dirname + '/widgets/syncthing/js/syncthing.js', __dirname + '/widgets/syncthing/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/syncthing')) {
            fs.renameSync(__dirname + '/widgets/syncthing',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Syncthing/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Syncthing'
                        },
                        {
                            match: /Joerg Zdarsky/g,
                            replacement: author
                        },
                        {
                            match: /joerg.zdarsky@gmx.de/g,
                            replacement: email
                        }
                    ]
                },
